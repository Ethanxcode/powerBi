...
module Blazer
  class Check < Record
    ...
    def update_state(result)
      ...
      self.last_run_at = Time.zone.now if respond_to?(:last_run_at=)
      ...
      if (state_was != "new" || state != "passing") && state != state_was
        Blazer::CheckMailer.state_change(
          self, state, state_was, result.rows.size, message, result.columns,
          result.rows.first(10).as_json, result.column_types, check_type).deliver_now if emails.present?
