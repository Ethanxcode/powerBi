...
class SyncTimesRecord < ApplicationRecord
  ...
  def self.update_sync_time(record_name, ip_address)
    transaction do
      ...
      if last_sync_time.new_record?
        Rails.logger.info "Last sync time by #{ip_address} for table #{table_name} created successfully."
        # flash[:notice] = "Last sync time by #{ip_address} for table #{table_name} created successfully."
      else
        Rails.logger.info "Last sync time by #{ip_address} for table #{table_name} updated successfully."
      end
      last_sync_time.save!
    end
