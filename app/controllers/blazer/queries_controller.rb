...
module Blazer
  class QueriesController < BaseController
    ...
    def show
      ...
      if @success
        @run_data = {
          statement: @query.statement, query_id: @query.id, data_source: @query.data_source,
          variables: variable_params(@query)
        }
    ...
    def run
      ...
      if query_running
        ...
        if @success
          ...
          render_run
        elsif Time.zone.now > Time.zone.at(@timestamp + (@data_source.timeout || 600).to_i + 5)
          # query lost
        ...
        options = { user: blazer_user, query: @query, refresh_cache: params[:check], run_id: @run_id, async: }
        if async && request.format.symbol != :csv
          Blazer::RunStatementJob.perform_later(
            @data_source.id, @statement.statement,
            options.merge(values: @statement.values))
    ...
    private
      ...
      def render_run
        ...
        @min_width_types = @columns.each_with_index.select { |c, i|
   @first_row[i].is_a?(Time) || @first_row[i].is_a?(String) || @data_source.smart_columns[c] }.map(&:last)
        ...
        respond_to do |format|
          ...
          format.csv do
            ...
            send_data data, type: "text/csv; charset=utf-8", disposition: "attachment", filename:
          end
        end
      end
      ...
      def map_tooltip(r)
        r.map { |v, i|
   "<strong>#{ERB::Util.html_escape(@columns[i])}:</strong> #{ERB::Util.html_escape(v)}" }.join("<br>").truncate(140, separator: " ")
      end
   ...
   !q.name.to_s.start_with?("#") || q.try(:creator).try(:id) == blazer_user.try(:id) }
      ...
      def blazer_time_value(data_source, k, v)
        data_source.local_time_suffix.any? { |s|
   k.ends_with?(s) } ? v.to_s.sub(" UTC", "") : v.in_time_zone(Blazer.time_zone)
      end
