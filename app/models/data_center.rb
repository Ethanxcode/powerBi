...
class DataCenter < ApplicationRecord
  ...
  def self.insert_from_api(result)
    ...
    if result.is_a?(Hash) && result["data"].is_a?(Array)
      ...
      puts "Starting transaction for data import"
      ...
      puts "Finished transaction for data import"
    else
      Rails.logger.warn "Result is not a hash or does not contain an array of data"
      puts "Result is not a hash or does not contain an array of data"
    end
  ...
  def self.auto_insert_from_api(api_url)
    ...
    if response
      begin
        ...
        puts "Data insertion complete."
        SyncTimesRecord.update_sync_time("data_center")
        nil
      rescue StandardError => e
        puts "Error updating last sync time for table data_center #{e.message}"
        SyncTimesRecord.create(status: "failure", description: error[:errors].join(", "))
      end
    else
    ...
    Log.create(status: "failure", description: e.message) # Add log to database
