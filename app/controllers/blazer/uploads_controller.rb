      ...
      def update_file(upload, drop: nil)
        ...
        begin
          ...
          Blazer.uploads_connection.transaction do
            ...
            Blazer.uploads_connection.execute(
              "CREATE TABLE #{upload.table_name} (#{columns.map.with_index { |c, i|
 "#{Blazer.uploads_connection.quote_column_name(c)} #{column_types[i]}" }.join(", ")})")
